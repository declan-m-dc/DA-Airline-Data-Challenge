...
Required packages for this script are **tidyverse**, **readxl**, and **knitr**; 
the import functions and output are suppressed for improved readability. 
...
based on the origin and destination airports. 
...
total of all fares, and take the mean value of the airport cost calculated above
(for later use).
...
```{r Sort and return busiest routes}

Busiest_Round_Trips <- Round_Trips_Summary_Expanded %>%
  ungroup() %>%
  slice_max(n=10, order_by = total_passengers)

kable(Busiest_Round_Trips$Route, col.names = "Busiest Routes")
```
...
In a previous steps, I already normalized the delay fields to only return the
...
multiply them by 75 per-minute cost, and sum the departure and arrival costs.

In earlier steps, I already created the Airport Cost field for each route, so 
...
Total fares have been created in a prior step using the ticket dataset.
...
kable(Most_Profitable_Routes$Route, col.names = "Most Profitable Routes")
```

This information is already represented here in a bar graph (below),
arranged by greatest total revenue. 
...
Graph1 <- ggplot(Most_Profitable_Routes_bar) +
